...
//         if (pipe(fd) == -1) {
//             perror("[Error]: pipe call failed");
//             return;
//         }
        
//         pid_t id = fork();
//         if (id == -1) {
//             perror("[Shell Error]: fork call failed");
//             return;
//         }
...
//                 free(input_line);
//                 free(command_args);
//                 perror("[Shell Error]: Invalid input for pipeline");
//             }

//             // handle after pipeline commands
//             if (fd[1] != -1) {
//                 char *p = command_args;
...
//                 while (p) { p++; size++; }
//                 command_args = realloc(command_args, (size + 1) * sizeof(char *));

//                 if (read(fd[0], buffer, sizeof(buffer) - 1) == -1) {
//                     perror("[Shell Error]: read syscall failed");
//                     return;
//                 }
//                 command_args[size] = buffer;
//             }
